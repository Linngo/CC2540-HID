...
//  uint8 data =0;
	...
	EPD_W21_Write(SetRamX, sizeof(SetRamX));	// Set RamX
	EPD_W21_Write(SetRamY, sizeof(SetRamY));	// Set RamY	
	ReadBusy();
...
{	
	...
	EPD_W21_Write(analogblock, sizeof(analogblock));	// set analog block control
	EPD_W21_Write(digitalblock, sizeof(digitalblock));	// set digital block control      
	EPD_W21_Write(Driveroutput, sizeof(Driveroutput));	// Driver output control    
      EPD_W21_Write(RamDataEntryMode, sizeof(RamDataEntryMode));	// X increase, Y decrease  
      EPD_W21_Write(choiceRam, sizeof(choiceRam));	// choice Ram 
	EPD_W21_Write(Ramxadress, sizeof(Ramxadress));	// Ram-X address
	EPD_W21_Write(Ramyadress, sizeof(Ramyadress));	// Ram-Y address  
	EPD_W21_Write(Setborder, sizeof(Setborder));	// set border 
	

	EPD_W21_WriteCMD_p1(0x18,0x80);
 	EPD_W21_WriteCMD_p1(0x22,0xB1);
	EPD_W21_WriteCMD(0x20);
